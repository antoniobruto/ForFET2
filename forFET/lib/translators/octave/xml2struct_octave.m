function  outStruct  = xml2struct_octave(input)
...
if isa(input, 'org.apache.xerces.dom.DeferredDocumentImpl') ||...
    ...
    try 
        if exist(input, 'file') == 2
            % xDoc = xmlread(input);
            javaaddpath('xercesImpl-2.11.0.jar');
            javaaddpath('/Users/kekatos/Downloads/xerces-2_12_1-xml-schema-1.1');

            %fprintf('Added to path\n');
            javaaddpath('xml-apis-1.4.01.jar');
...
if theNode.hasChildNodes%(theNode)
    childNodes = theNode.getChildNodes();%(theNode);
    numChildNodes = childNodes.getLength();%(childNodes);
%     whos numChildNodes
%     whos childNodes
...
if theNode.hasAttributes
   ...
   for count = 1:numAttributes
        ...
        str = char(toString(theAttributes.item(count-1)));
        str = char((theAttributes.item(count-1)));
        warning('Deleted function');
        fprintf('%s \n',str);
