...
public:
  ...
  explicit MatrixDoubletonSet(size_type);                                            ///< D:=Id, R0:=0, C:=Id, R:=0, B:=Id
  ...
  using BaseSet::m_R;
  using BaseSet::m_invBjac;
  /// r0 is a interval set in given coordinate system that represents 'Lipschitz' part
//protected:
  MatrixType m_R0;
public:
  const MatrixType & get_R0 () const {return m_R0;}
    const ScalarType & getElement_R0 (size_type i, size_type j) const {return m_R0(i+1,j+1);}
    VectorType getRow_R0 (size_type i) const {return m_R0.row(i);}
    ColumnVectorType getColumn_R0 (size_type j) const {return m_R0.column(j);}
    void set_R0 (const MatrixType & R0) {m_R0 = R0;}
    void setElement_R0 (size_type i, size_type j, const ScalarType & s) {m_R0(i+1, j+1) = s;}
    template <typename VectorT>
    void setRow_R0 (size_type i, const VectorT & v) {m_R0.row(i) = v;}
    template <typename VectorT>
    void setColumn_R0 (size_type j, const VectorT & v) {m_R0.column(j) = v;}
...
public:
  ...
  const ScalarType & getElement_Cjac (size_type i, size_type j) const {return m_Cjac(i+1,j+1);}
  VectorType getRow_Cjac (size_type i) const {return m_Cjac.row(i);}
  ColumnVectorType getColumn_Cjac (size_type j) const {return m_Cjac.column(j);}
  void set_Cjac (const MatrixType & C) {m_Cjac = C;}
  void setElement_Cjac (size_type i, size_type j, const ScalarType & s) {m_Cjac(i+1, j+1) = s;}
  template <typename VectorT>
  void setRow_Cjac (size_type i, const VectorT & v) {m_Cjac.row(i) = v;}
  template <typename VectorT>
  void setColumn_Cjac (size_type j, const VectorT & v) {m_Cjac.column(j) = v;}
