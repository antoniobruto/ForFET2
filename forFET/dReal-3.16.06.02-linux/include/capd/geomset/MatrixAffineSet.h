...
public:
  ...
  explicit MatrixAffineSet(size_type dim);                                                 // D:=Id,      Bjac:=Id  R:=0,
...
public:
  ...
  const ScalarType & getElement_D (size_type i, size_type j) const {return m_D(i+1,j+1);}
  VectorType getRow_D (size_type i) const {return m_D.row(i);}
  ColumnVectorType getColumn_D (size_type j) const {return m_D.column(j);}
  void set_D (const MatrixType & Bjac) {m_D = Bjac;}
  void setElement_D (size_type i, size_type j, const ScalarType & s) {m_D(i+1, j+1) = s;}
  template <typename VectorT>
  void setRow_D (size_type i, const VectorT & v) {m_D.row(i) = v;}
  template <typename VectorT>
  void setColumn_D (size_type j, const VectorT & v) {m_D.column(j) = v;}
...
public:
  ...
  const ScalarType & getElement_R (size_type i, size_type j) const {return m_R(i+1,j+1);}
  VectorType getRow_R (size_type i) const {return m_R.row(i);}
  ColumnVectorType getColumn_R (size_type j) const {return m_R.column(j);}
  void set_R (const MatrixType & Bjac) {m_R = Bjac;}
  void setElement_R (size_type i, size_type j, const ScalarType & s) {m_R(i+1, j+1) = s;}
  template <typename VectorT>
  void setRow_R (size_type i, const VectorT & v) {m_R.row(i) = v;}
  template <typename VectorT>
  void setColumn_R (size_type j, const VectorT & v) {m_R.column(j) = v;}
...
public:
  const MatrixType & get_invBjac () const {return m_invBjac;}
  const MatrixType & get_Bjac () const {return m_Bjac;}
  const ScalarType & getElement_Bjac (size_type i, size_type j) const {return m_Bjac(i+1,j+1);}
  VectorType getRow_Bjac (size_type i) const {return m_Bjac.row(i);}
  ColumnVectorType getColumn_Bjac (size_type j) const {return m_Bjac.column(j);}
  void set_Bjac (const MatrixType & Bjac) {m_Bjac = Bjac;}
  void setElement_Bjac (size_type i, size_type j, const ScalarType & s) {m_Bjac(i+1, j+1) = s;}
  template <typename VectorT>
  void setRow_Bjac (size_type i, const VectorT & v) {m_Bjac.row(i) = v;}
  template <typename VectorT>
  void setColumn_Bjac (size_type j, const VectorT & v) {m_Bjac.column(j) = v;}
