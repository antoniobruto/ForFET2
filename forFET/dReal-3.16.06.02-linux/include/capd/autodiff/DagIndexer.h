...
inline __size_type index(__size_type dim, __size_type j, __size_type c)
{
  ...
  // Remark: c is an size_typeeger! Therefore we use bit shifts and compute equivalent (c+1)(c+2dim)/2
  return
...
private:
  ...
  ScalarType* m_coefficients; ///< posize_typeer to allocated memory
  ...
  JetSize m_timeJetSize;     ///< size of chunk of memory needed for (m_order+1) of jets.
};
