...
protected:
  ...
  void evalHomogenousPolynomial(size_type degree) const {
    for(unsigned i=0;i<this->m_nodes.size();++i)
      this->m_nodes[i]->evalHomogenousPolynomial(degree,this->dimension(),capd::autodiff::Order(this->getOrder()+1));
  }
  ...
  void eval(size_type degree, size_type coeffNo) const {
    for(unsigned i=0;i<this->m_nodes.size();++i)
      this->m_nodes[i]->eval(degree,this->dimension(),capd::autodiff::Order(this->getOrder()+1),coeffNo);
  }
...
void BasicFunction<Scalar>::reset(Function f, int dimIn, int dimOut, int noParam)
{
  ...
  Int4ToAbstractNode(this->m_evalPath,this->m_nodes,this->m_dag.timeJetSize(),this->m_dag.coefficients());
