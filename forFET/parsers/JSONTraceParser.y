%{	
	...
	void yyerror(char *);
	extern int yylex();
	extern int JT_lineNo,JT_charPos,yyleng,yyJT_lineno,yychar,JT_oldPos;
	extern char JT_line[MAX_STR_LENGTH],JT_oldLine[MAX_STR_LENGTH];
	extern double JT_maxNumber;
	extern float maxNumber;
	
	extern FILE *yyin;
	extern char* yytext;
	int JT_errNo,JT_err1No,JT_err2No,JT_err3No,JT_err4No,JT_err5No,JT_err6No;
	int JT_currentMatch;
...
text: text text	
	...
	| LBRACK interval RBRACK TEXT {
				...
				printf("FOUND INTERVAL for %s\n",$4);
			#endif
		fprintf(drhOut,"[%f,%f] %s",JT_maxNumber,JT_maxNumber,$4);
	}
	;
...
void yyerror(char *s) {
	...
	if(strlen(JT_line)==0){
		strcpy(JT_line,JT_oldLine);
		JT_charPos = JT_oldPos;
		JT_lineNo--;
	}
	if(flag==1){
		JT_currentMatch = JT_charPos+1;
	}
	//fprintf(stderr, "%s at Line %d ; %s Match [%d], JT_line :[%s],  ERROR CODES: [%d,%d,%d,%d,%d,%d,%d]\n", s,JT_lineNo,errorMsg,JT_currentMatch,JT_line,JT_errNo,JT_err1No,JT_err2No,JT_err3No,JT_err4No,JT_err5No,JT_err6No);
	fprintf(stderr, "%s at Line %d Character Position : [%d], Token = [%s]\n%s \n", s,JT_lineNo,JT_charPos+1,yytext,errorMsg);
	char *line = trim(JT_line);
	fprintf(stderr, "%s\n",line);
	int i=0;
	for(i=1;i<JT_currentMatch;i++){
		printf(" ");
	}
	printf("^\n");
	exit(0);
